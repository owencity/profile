...
export const portfolio = {
  ...
  introParagraphs: [
    ...
    '연매출 250억 규모의 유통업 회사에서 10년된 레거시 기술 부채를 해결하는 마이그레이션 프로젝트를 주도했습니다. 타사에서 인계받은 노후 소스코드와 50여 개의 복잡한 상품 데이터 테이블을 전수 분석하여, 도메인에 맞게 30개 수준으로 통합·재설계했습니다. 추가 개발이 불가능했던 상품관리시스템을 최신 아키텍처로 마이그레이션하고, 안정적인 자사몰 API 연동과 모니터링 환경을 구축하여 서비스 장애율을 낮췄습니다.',
  ],
  contact: {
  ...
  education: {
    ...
    degree: '컴퓨터공학 학사 (학점은행제 · 국가평생교육진흥원) · 2027.03 졸업 예정',
  ...
  projectGroups: [{
    ...
    projects: [
    {
      ...
      summary: [
        ...
        '3개월 분석 후 작은 기능부터 검증하며 전환을 진행해 약 2개월 만에 1차 개발을 완료했고, 이후로도 기능 고도화를 이어가며 신규 기능 개발 기간을 1주일에서 2~3일로 줄였습니다.',
      ],
      diagram: {
      ...
      problem:
        '입사 3개월 차에 타사에서 인계받은 노후 소스코드(Java 1.6·Seasar2·MySQL 5)로 레거시 시스템 개발을 맡았는데, 신규 기능을 추가하려 할 때마다 노후된 프레임워크의 한계에 부딪혔습니다. 이대로는 안 되겠다고 판단해 직접 마이그레이션을 제안하고 PoC로 실현 가능성을 검증한 뒤 전환을 주도했습니다. Seasar2는 서버사이드 렌더링 구조라 화면단을 React로 완전히 분리하는 아키텍처 전환이 필요했고, 기존과 똑같이 동작해야 한다는 제약 속에서 기능 하나 놓치지 않고 옮기는 게 과제였습니다.',
      solutionGroups: [
        {
          ...
          items: [
            {
              ...
              desc: '인계받은 코드를 3개월간 분석하며 시스템 흐름과 기능을 익혔고, 작은 기능 하나를 먼저 마이그레이션해 실현 가능성을 확인한 뒤 본격적으로 전환을 진행했습니다. 실행 후 1차 개발 완료까지 약 2개월이 걸렸고, 이후로도 기능 고도화를 계속 이어갔습니다.',
            },
          ],
        },
        {
          ...
          items: [
            {
              ...
              desc: '빌드·배포 로직을 셸 스크립트(sh)에 정리하고 Docker로 감싸, Jenkins가 이 스크립트를 실행해 Staging(dev)에 먼저 배포·검증한 뒤 Prod(IDC 서버)로 넘기는 파이프라인을 구성했습니다. 운영 서버로 바로 배포하지 않고 검증 단계를 하나 거치도록 한 것으로, 수동으로 진행하던 배포가 자동화되며 소요 시간이 20분에서 5분으로 줄었습니다.',
            },
          ],
        },
      ],
      result: {
        before: '신규 기능 개발 1주일',
        after: '2~3일',
        note: '배포도 Jenkins·Docker 도입으로 20분에서 5분으로 줄었습니다. 별도로 측정한 수치가 아니라 실무를 진행하며 직접 확인한 수치입니다. 기능을 더 빠르게 반영할 수 있게 되면서, 상품관리 시스템을 쓰는 담당 부서의 업무 요청 처리 속도도 함께 빨라졌습니다.',
      },
    },
    {
      ...
      result: {
        note: '장애 1회로 연동 파이프라인 전체가 지연되던 구조에서, 실패 격리·자동 재시도·알람까지 갖춘 구조로 전환했습니다. 자사몰에 상품 정보가 실시간으로 연동되면서, 고객이 보는 가격·상품 정보의 최신성도 좋아졌습니다.',
      },
    },
    {
    ...
    },
    {
      ...
      problem:
        '대량의 상품 데이터를 엑셀로 업로드할 때 두 가지 문제가 있었습니다. 업로드된 엑셀 파일을 통째로 메모리에 올려서 처리하다 보니 행 수가 많을수록 메모리를 많이 썼고, 업로드 중간에 일부 행만 저장된 채로 실패하면 어디까지 반영됐는지 알 수 없어 재실행도 위험했습니다. 메모리를 적게 쓰면서도, 실패하면 안전하게 되돌릴 수 있는 업로드 파이프라인을 만드는 게 과제였습니다.',
      solutionGroups: [
        {
          title: '메모리 사용량 최소화',
          items: [
            {
              ...
              desc: '이전에는 엑셀 파일 전체를 한 번에 객체로 만들어 메모리에 올렸는데, 행 수가 많을수록 메모리를 더 많이 썼습니다. 그래서 파일을 한 번에 다 읽지 않고, 행을 하나씩 읽을 때마다 그 행의 값을 정의된 컬럼에 맞게 매칭하고 형식을 변환하는 처리를 바로 진행하도록 바꿨습니다. 저장도 한 건씩 하지 않고 1,000건씩 묶어서 진행해, 순서를 유지하면서 진행률도 보여줍니다. 다만 저장 전에 전체 데이터를 한 번 더 검증해야 해서 그 순간에는 파싱 결과 전체가 메모리에 모이는데, 이 부분은 숨기지 않고 앞으로 개선할 지점으로 남겨뒀습니다.',
              diagram: {
          ...
          items: [
            {
              ...
              desc: '예를 들어 1,000행 중 700행까지 저장되고 701번째 행에서 실패하면, 어디까지 반영됐는지 알 수 없어 재실행 시 이미 저장된 700행이 중복으로 다시 들어갈 위험이 있습니다. 그래서 하나라도 실패하면 지금까지 반영한 것까지 전부 되돌려 업로드 전 상태로 완전히 되돌아가는 전량 원복 정책으로 고정했습니다. 이러면 몇 번을 재실행해도 항상 "0건 반영" 상태에서 다시 시작하니 안전합니다. 다만 그 대가로 999개 행이 멀쩡해도 1개 행의 타입 오류 때문에 999개까지 전부 롤백됩니다. 그래서 저장을 시도하기 전에 타입·형식을 미리 검증하는 단계를 앞에 둬서, 문제 있는 행은 저장 시도 자체를 하지 않고 그 행만 반려하도록 했습니다. 저장(트랜잭션)에는 이미 검증을 통과한 행만 들어가므로, 가장 흔한 실패 원인은 트랜잭션에 들어가기도 전에 걸러지고 전량 원복 정책은 그대로 유지됩니다. 원복 전에는 실패 사유를 로그로 남겨 원인을 추적할 수 있게 했습니다.',
              diagram: {
    ...
    },
    {
      ...
      problem:
        '엑셀 업로드 파이프라인에는 서로 다른 원인의 사고가 반복됐습니다. 양식의 컬럼 위치가 바뀌면 이를 인식하지 못해 상품코드가 통째로 유실됐고(106행×4회 반려), 검증은 통과한 값도 저장 시 형식이 바뀌며 브랜드코드 매칭이 실패했으며(1,114행), 화면과 저장 로직이 서로 다른 기준으로 등록 여부를 판단해 결론이 어긋나는 사고도 있었습니다(24행 반려). 이런 사고가 재발하지 않는 구조로 파이프라인을 다시 짜는 게 과제였습니다.',
      solutionGroups: [
        {
          title: '위치 의존성 제거',
          items: [
